...
import { RiPlaneLine, RiSearchEyeLine, RiArrowLeftLine, RiCalendarLine, RiTimerLine, RiArrowRightLine, RiLoader4Line } from "@remixicon/react"
...
function SearchResults() {
  ...
  React.useEffect(() => {
    if (origin || destination || dateStr || selectedClass) {
      setSearchState({
        origin,
        destination,
        date: dateStr,
  ...
  }, [origin, destination, dateStr, selectedClass, setSearchState])
  ...
  React.useEffect(() => {
    async function searchFlights() {
      ...
      try {
        ...
        if (dateStr) {
          // departs_at is TIMESTAMPTZ, match start to end of search date in UTC/ISO
        ...
        setErrorMsg("Failed to query flight schedules. Please try again.")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  return (
    <div className="flex-grow container mx-auto px-4 md:px-6 py-10 space-y-8 min-h-[75vh]">
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-sm font-medium text-zinc-400">Locating matching AeroLux schedules...</p>
          </div>
        ) : errorMsg ? (
          ...
          <div className="space-y-4">
            <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-400">Available Departures ({flights.length})</h2>
      ...
      {/* Visual Seat Selection Overlay Drawer Modal */}
      {activeFlight && (
...
export default function SearchPage() {
  return (
    <React.Suspense fallback={
      <div className="flex-grow container mx-auto px-4 md:px-6 py-20 text-center space-y-4">
        <RiLoader4Line className="h-10 w-10 text-primary animate-spin mx-auto" />
        <p className="text-zinc-500 text-sm">Preparing search parameters...</p>
      </div>
    }>
      <SearchResults />
    </React.Suspense>
